        ...
        <div class="main-page">
            ...
            <div class="grids">
                ...
                <div class="panel panel-widget">
                    <div class="row">
                        <label class="col-md-2">矩阵行数:</label>
                        <div class="col-md-8">
                            <input id="row" type="number" value="3"/>
                        </div>
                    </div>
                    <div class="row">
                        <label class="col-md-2">矩阵列数:</label>
                        <div class="col-md-8">
                            <input id="col" type="number" value="3"/>
                        </div>
                    </div>
                    <hr/>
                    ...
                    <hr/>
                    <div>原矩阵的秩:<span class='value'></span></div>
                    <div>逆矩阵:</div>
                    <div class="row"><!--<td><input class="text-center" type="text" />0</td>-->
                        <table class="nifz">
...
//克隆的元素
  ...
  //改变矩阵
	...
	var fun = function(){
	   	...
	   	var arr1 = [];
		   var item = $('input', fzobj);
	    for(var i = 0;i < item.length;i++){
	        var txt = $(item.get(i)).val();
	       	arr1[i] = txt;
	     }
	    ajax(
				{
					url:"<c:url value='/ojs.action!nizzz'/>",
					type:"json",
					method:"post",
					params:"arr=" + arr1 + "&row=" + rowobj.val() + "&col=" + colobj.val(),
					callback:function(data){
						$('.value').html(data[0]);
						changetable(nifz, data[1].row, data[1].col, data[1].data);
						changetable(zzfz, data[2].row, data[2].col, data[2].data);
					}
				}
			);
	};
	...
	//fzobj
    rowobj.change(function(){
    	changetable(fzobj, rowobj.val(), colobj.val());
    	$('input', fzobj).change(fun);
    });
    colobj.change(function(){
    	changetable(fzobj, rowobj.val(), colobj.val());
    	$('input', fzobj).change(fun);
    });
    $('input', fzobj).change(fun);
