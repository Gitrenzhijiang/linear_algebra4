                ...
                <div class="compose-right widget-shadow">
                    <div class="panel-default">
                        <div class="panel-heading">
                            提交作业:${requestScope.task.name }
                        </div>
                        <div class="panel-body">
                            <div class="alert alert-info">
                                <c:choose>
                                	<c:when test="${ not empty requestScope.form_err }">${requestScope.form_err }</c:when>
                                	<c:otherwise>请认真填写</c:otherwise>
                                </c:choose>
                            </div>
                            
                            <form class="com-mail" action="<c:url value='/task.action!submit'/>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="task_id" value="<c:choose>
                            	<c:when test="${not empty requestScope.task }">${requestScope.task.pk_id }</c:when>
                            	<c:when test="${not empty requestScope.taskfinish.task }">${requestScope.taskfinish.task.pk_id }</c:when>
                            </c:choose>">

                                <textarea rows="6" name="res" class="form-control1 control2" placeholder="作业:">${requestScope.taskfinish.res }</textarea>
                                <div class="form-group">
                                    <div class="btn btn-default btn-file">
                                        <i class="fa fa-paperclip"></i>
                                        <input type="file" name="filename">作业附件(非必需)
                                    </div>
