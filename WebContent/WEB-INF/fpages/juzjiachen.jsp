...
<div class="main-content">
    ...
    <div id="page-wrapper">
        <div class="main-page">
            <!--grids-->
            <table id='clonediv' class="hidden">
				<td id='td'>
					<input class="text-center" style='max-width: 60px;' type="number" value="0" />
				</td>
				<tr id='tr'></tr>
			</table>
            <div class="grids">
                ...
                <div class="panel panel-widget">
                    <div class="row">
                        <label class="col-md-2">矩阵A行数:</label>
                        <div class="col-md-8">
                            <input id="arow" type="number" value="3"/>
                        </div>
                    </div>
                    <div class="row">
                        <label class="col-md-2">矩阵A列数:</label>
                        <div class="col-md-8">
                            <input id="acol" type="number" value="3"/>
                        </div>
                    </div>
                    <hr/>
                    <div class="row"><!--<td><input class="text-center" type="text" />0</td>-->
                        <table class="afz">
                            <tr>
                                <td><input class="text-center" style="max-width: 60px;" type="number" value="0" /></td>
                                <td><input class="text-center" style="max-width: 60px;" type="number" value="0" /></td>
                                <td><input class="text-center" style="max-width: 60px;" type="number" value="0" /></td>
                            </tr>
                            <tr>
                    ...
                    <hr/>
                    <div class="row">
                        <label class="col-md-2">矩阵B行数:</label>
                        <div class="col-md-8">
                            <input id="brow" type="number" value="3"/>
                        </div>
                    </div>
                    <div class="row">
                        <label class="col-md-2">矩阵B列数:</label>
                        <div class="col-md-8">
                            <input id="bcol" type="number" value="3"/>
                        </div>
                    </div>
                    <hr/>
                    <div class="row"><!--<td><input class="text-center" type="text" />0</td>-->
...
<script>
    ...
    var acolobj = $('#acol');
   	...
   	var btb = $('.bfz');
   	//克隆的元素
   	var trobj = $('#tr', $('#clonediv'));
   	var tdobj = $('#td', $('#clonediv'));

	arowobj.change(function(){
		changetable(atb, arowobj.val(), acolobj.val());
		$('input', atb).change(fun);
	});
	acolobj.change(function(){
		changetable(atb, arowobj.val(), acolobj.val());
		$('input', atb).change(fun);
	});
	browobj.change(function(){
		changetable(btb, browobj.val(), bcolobj.val());
		$('input', btb).change(fun);
	});
	bcolobj.change(function(){
		changetable(btb, browobj.val(), bcolobj.val());
		$('input', btb).change(fun);
	});
   	...
   	//改变矩阵
   	var changetable = function(tb, row, col, arr){
   		tb.empty();
   	   if(row > 0 && col > 0){
   		
   	       for(var i = 0;i < row;i++){
   			var kltr = trobj.get(0).cloneNode(false);
   			
   			tb.append($(kltr));
   	           
   	           for(var j = 0;j < col;j++){
   	               var kltd = tdobj.get(0).cloneNode(true);
   	               if(arr != null){
   	            		$('input',kltd).val(arr[i*col+j]);
   	   	            }
   	               $(kltr).append($(kltd));
   	           }
   	       }
    ...
    var fun = function(){
	    ...
	    for(var i = 0;i < item2.length;i++){
	        var txt = $(item2.get(i)).val();
	       	arr2[i] = txt;
	     }

	     ajaxfun(arr1,  arowobj.val(),acolobj.val(), arr2, browobj.val(),bcolobj.val() );
	     
     };
  

     var ajaxfun = function(arr1, arow, acol, arr2, brow, bcol){
    	 	ajax(
    				{
    					url:"<c:url value='/ojs.action!juzjiachen'/>",
    					type:"json",
    					method:"post",
    					params:"arr1=" + arr1 + "&arr2=" + arr2 + "&arow=" + arow + "&acol="
    						+ acol + "&brow="+brow +"&bcol=" + bcol,
    					callback:function(data){
    						changetable($('.res1'), data[0].row, data[0].col, data[0].data);
    						changetable($('.res2'), data[1].row, data[1].col, data[1].data);
    					}
    				}
    			);
