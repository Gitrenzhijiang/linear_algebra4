                    ...
                    <div class="panel panel-widget">
                        <div class="compose-left">
                            <div class="folder widget-shadow">
                                <ul>
                                    <li class="head">全部问题</li>
                                    <li><a href="<c:url value='/user.action!toindex?issolved=0'/>"><i class="fa fa-inbox"></i>未解决 </a></li>
                                    <li><a href="<c:url value='/user.action!toindex?issolved=1'/>"><i class="fa fa-file-text-o"></i>已解决</a></li>
                                    <li><a href="<c:url value='/user.action!toindex?level=0'/>"><i class="fa fa-file-text-o"></i>简单</a> </li>
                                    <li><a href="<c:url value='/user.action!toindex?level=1'/>"><i class="fa fa-file-text-o"></i>中等</a></li>
                                    <li><a href="<c:url value='/user.action!toindex?level=2'/>"><i class="fa fa-flag-o"></i>难</a></li>
                                    <li><a href="<c:url value='/user.action!toindex?level=3'/>"><i class="fa fa-flag-o"></i>困难</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="inbox-page">
						<c:forEach items="${requestScope.problemList }" var="problem">
	                        <div class="inbox-row widget-shadow"  role="tablist" aria-multiselectable="true">
	                            <div class="mail"><img src="${problem.user.headpic }" alt=""></div>
	                            <div class="mail mail-name"> <h6>${problem.user.name }</h6> </div>
	                            <a role="button"  href="<c:url value='/question.action!toanswerdiscuss?problem_id='/>${problem.pk_id }" >
	                                <div class="mail"><p>${problem.title }</p></div>
	                            </a>
	                            <div class="mail-right"><p><fmt:formatDate value="${problem.createtime }" pattern="yyyy-MM-dd HH:mm:ss"/></p></div>
	                            <div class="clearfix"> </div>
	                        </div>
						</c:forEach>
						<c:choose>
	                        ...
	                        <div class="text-center">
	                            <nav>
	                                ...
	                                <ul class="pagination"> 
	                                    <li <c:if test="${requestScope.pagebean.isfirst}">class="disabled"</c:if> ><a href="<c:url value='/user.action!toindex?pageindex=${requestScope.pagebean.preindex }&level=${level }&issolved=${issolved }&user_id=${user_id }'/>" aria-label="Previous"><i class="fa fa-angle-left"></i></a></li>
	                                    <c:forEach items="${requestScope.pagebean.pageList }" var="page"> 
	                                    	<li <c:if test="${requestScope.pagebean.pageindex eq page}">class="active"</c:if>><a href="<c:url value='/user.action!toindex?pageindex=${page }&level=${level }&issolved=${issolved }&user_id=${user_id }'/>">${page }</a></li>
	                                    </c:forEach>
	                                    <li <c:if test="${requestScope.pagebean.islast}">class="disabled"</c:if>><a href="<c:url value='/user.action!toindex?pageindex=${requestScope.pagebean.nextindex }&level=${level }&issolved=${issolved }&user_id=${user_id }'/>" aria-label="Next"><i class="fa fa-angle-right"></i></a></li>
	                                </ul>
	                            </nav>
	                        </div>
	                        </c:otherwise>
