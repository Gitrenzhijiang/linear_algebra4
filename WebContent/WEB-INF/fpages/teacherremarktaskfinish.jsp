                ...
                <div class="compose-right widget-shadow">
                    <div class="panel-default">
                        <div class="panel-heading">
                            评价 ${requestScope.taskfinish.task.name }&nbsp;${requestScope.taskfinish.userByUserId.user_no}&nbsp;
                            ${requestScope.taskfinish.userByUserId.name}
                        </div>
                        <div class="panel-body">
                            <div class="alert alert-info">
                                <c:choose>
                                	<c:when test="${requestScope.form_err }">${requestScope.form_err }</c:when>
                                	<c:otherwise>请详细填写评价内容</c:otherwise>
                                </c:choose>
                            </div>
                            <form class="com-mail" action="<c:url value='/task.action!remarktf'/>" method="post">
								<input type="hidden" name="pk_id" value="${requestScope.taskfinish.pk_id }"/>
                                <textarea rows="6" name="remark" class="form-control1 control2" placeholder="评价内容:">${taskfinish.remark }</textarea>
