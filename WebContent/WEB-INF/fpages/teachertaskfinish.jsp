                ...
                <div class="compose-grids">
                    ...
                    <div class="panel panel-widget">
                        <div class="inbox-page">
							<c:forEach items="${requestScope.tfList }" var="tf">
                            <div class="inbox-row widget-shadow" id="accordion" role="tablist" aria-multiselectable="true">

                                <div class="mail"><img src="${tf.userByUserId.headpic }" alt=""></div>
                                <div class="mail mail-name"> <h6>${tf.userByUserId.user_no }&nbsp;&nbsp;${tf.userByUserId.name }</h6> </div>
                                <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseOne${tf.pk_id }" aria-expanded="true" aria-controls="collapseOne${tf.pk_id }">
                                    <div class="mail"><p>
                                    	<c:choose>
                                    		<c:when test="${tf.status eq 1 }">已完成</c:when>
                                ...
                                <div class="mail-right">
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu float-right">
                                        	<c:if test="${tf.status eq 1 }">
                                            <li>
                                                <a role="button"  href="<c:url value='/task.action!toremarktf?tf_id=${tf.pk_id }'/>" >
                                                    <i class="fa fa-reply mail-icon"></i>
                                                   	 评价
                                                </a>
                                            </li>
                                            <c:if test="${not empty tf.filepath }">
                                            <li>
                                                <a href="<c:url value='/task.action!downloadTF?tf_id=${tf.pk_id }'/>" class="font-red" title="">
                                ...
                                <div class="mail-right"><p>
                                	<c:choose>
                                		<c:when test="${tf.status eq 1 }"><fmt:formatDate value="${tf.finishtime }" pattern="yyyy-MM-dd HH:mm:ss"/></c:when>
                                		<c:otherwise>未知</c:otherwise>
                                	</c:choose>
                                </p></div>
                                <div class="clearfix"> </div>
                                <div id="collapseOne${tf.pk_id }" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
                                    <div class="mail-body">
                                        <p>${tf.res }</p>
                           ...
                           <c:choose>
	                        ...
	                        <div class="text-center">
	                            <nav>
	                                <ul class="pagination"> 
	                                    <li <c:if test="${requestScope.pagebean.isfirst}">class="disabled"</c:if> ><a href="<c:url value='/task.action!totaskfinish?pageindex=${requestScope.pagebean.preindex }&task_id=${task_id }'/>" aria-label="Previous"><i class="fa fa-angle-left"></i></a></li>
	                                    <c:forEach items="${requestScope.pagebean.pageList }" var="page"> 
	                                    	<li <c:if test="${requestScope.pagebean.pageindex eq page}">class="active"</c:if>><a href="<c:url value='/task.action!totaskfinish?pageindex=${page }&task_id=${task_id }'/>">${page }</a></li>
	                                    </c:forEach>
	                                    <li <c:if test="${requestScope.pagebean.islast}">class="disabled"</c:if>><a href="<c:url value='/task.action!totaskfinish?pageindex=${requestScope.pagebean.nextindex }&task_id=${task_id }'/>" aria-label="Next"><i class="fa fa-angle-right"></i></a></li>
	                                </ul>
	                            </nav>
	                        </div>
	                        </c:otherwise>
