            ...
            <div class="grids">
                ...
                <div class="panel panel-widget">
                    ...
                    <div class="inbox-page">
                        <h4>未读通知</h4>
                        <c:forEach items="${requestScope.unreadNotices }" var="unotice">
                        <div class="inbox-row widget-shadow"  role="tablist" aria-multiselectable="true">
                            <div class="mail"><img src="${unotice.user.headpic }" alt=""></div>
                            <div class="mail mail-name"> <h6>${unotice.user.name }</h6> </div>
                            <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseOne${unotice.pk_id }" aria-expanded="true" aria-controls="collapseOne${unotice.pk_id }">
                                <div class="mail"><p>${unotice.ntitle }</p></div>
                            </a>
                            <div class="mail-right">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu float-right">
                                        <c:forEach items="${unotice.noticeisreads }" var="nir">
                                    		<c:if test="${nir.user_id eq sessionScope.user.pk_id }">
                                    			<c:if test="${nir.isread eq 0}">
                                    				<li>
                                            		<a href="<c:url value='/notice.action!already?notice_id=${unotice.pk_id}'/>" title="">
                            ...
                            <div class="mail-right"><p><fmt:formatDate value="${unotice.createtime }" pattern="yyyy-MM-dd HH:mm:ss"/></p></div>
                            <div class="clearfix"> </div>

                            <div id="collapseOne${unotice.pk_id }"  class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
                                <div class="mail-body">
                                    <p>${unotice.ntext }</p>
                                 </div>
                            </div>
                ...
                <div class="panel panel-widget">
                    ...
                    <div class="inbox-page">
                        <h4>所有通知</h4>
                       <c:forEach items="${requestScope.allNoticeList }" var="notice">
                        <div class="inbox-row widget-shadow"  role="tablist" aria-multiselectable="true">
                            <div class="mail"><img src="${notice.user.headpic }" alt=""></div>
                            <div class="mail mail-name"> <h6>${notice.user.name }</h6> </div>
                            <a role="button" data-toggle="collapse" data-parent="#accordion" href="#a${notice.pk_id }" aria-expanded="true" aria-controls="a${notice.pk_id }">
                                <div class="mail"><p>${notice.ntitle }</p></div>
                            </a>
                            <div class="mail-right">
                                <div class="dropdown">
                                    <a href="#" data-toggle="dropdown" aria-expanded="false">
                                    ...
                                    <ul class="dropdown-menu float-right">
                                    	...
                                    	<c:forEach items="${notice.noticeisreads }" var="nir">
                                    		<c:if test="${nir.user_id eq sessionScope.user.pk_id }">
                                    			<c:if test="${nir.isread eq 0}">
                                    				<li>
                                            		<a href="<c:url value='/notice.action!already?notice_id=${notice.pk_id}'/>" title="">
                                                	<i class="fa fa-reply mail-icon"></i>
                                                		设为已读
                                            		</a>
                                        			</li>
                            ...
                            <div class="mail-right"><p><fmt:formatDate value="${notice.createtime }" pattern="yyyy-MM-dd HH:mm:ss"/></p></div>
                            <div class="clearfix"> </div>

                            <div id="a${notice.pk_id }"  class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
                                <div class="mail-body">
                                    <p>${notice.ntext }</p>
                                 </div>
                            </div>
                        ...
                        <c:choose>
	                        ...
	                        <div class="text-center">
	                            <nav>
	                                <ul class="pagination"> 
	                                    <li <c:if test="${requestScope.pagebean.isfirst}">class="disabled"</c:if> ><a href="<c:url value='/notice.action!toallnotice?pageindex=${requestScope.pagebean.preindex }'/>" aria-label="Previous"><i class="fa fa-angle-left"></i></a></li>
	                                    <c:forEach items="${requestScope.pagebean.pageList }" var="page"> 
	                                    	<li <c:if test="${requestScope.pagebean.pageindex eq page}">class="active"</c:if>><a href="<c:url value='/notice.action!toallnotice?pageindex=${page }'/>">${page }</a></li>
	                                    </c:forEach>
	                                    <li <c:if test="${requestScope.pagebean.islast}">class="disabled"</c:if>><a href="<c:url value='/notice.action!toallnotice?pageindex=${requestScope.pagebean.nextindex }'/>" aria-label="Next"><i class="fa fa-angle-right"></i></a></li>
	                                </ul>
	                            </nav>
	                        </div>
	                        </c:otherwise>
