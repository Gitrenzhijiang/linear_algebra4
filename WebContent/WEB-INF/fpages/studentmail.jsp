                        ...
                        <div class="panel panel-widget">
                            <div class="compose-left">
                                <div class="folder widget-shadow">
                                    <ul>
                                        <li class="head">分类</li>
                                        <li><a href="<c:url value='/mail.action!tomail?isread=0'/>"><i class="fa fa-inbox"></i>未读 </a></li>
                                        <li><a href="<c:url value='/mail.action!tomail?isread=1'/>"><i class="fa fa fa-envelope-o"></i>已读</a></li>
                                        <li><a href="<c:url value='/mail.action!tomail?sended=1'/>"><i class="fa fa-file-text-o"></i>已发送</a> </li>
                    ...
                    <div class="col-md-9">
                        <div class="panel panel-widget">
                            <div class="inbox-page">
								<c:forEach items="${requestScope.inboxList }" var="inbox">
                                	<div class="inbox-row widget-shadow" id="accordion" role="tablist" aria-multiselectable="true">
                                    <div class="mail"><img src="${inbox.userBySendId.headpic }" alt=""></div>
                                    <div class="mail mail-name"> <h6>
                                    	<c:choose>
                                    	<c:when test="${inbox.userBySendId.pk_id eq sessionScope.user.pk_id}">
                                    		我(To:${inbox.userByReceiveId.name})
                                    	</c:when>
                                    	<c:otherwise>
                                    		${inbox.userBySendId.name }
                                    	</c:otherwise>
                                    	</c:choose>
                                    </h6>
                                     </div>
                                    <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseOne${inbox.pk_id }" aria-expanded="true" aria-controls="collapseOne${inbox.pk_id }">
                                        <div class="mail"><p>${inbox.subject }</p></div>
                                    </a>
                                    <div class="mail-right">
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu float-right">
                                            	<c:if test="${inbox.receive_id eq sessionScope.user.pk_id }">
	                                            	<li>
	                                                    <a role="button"  href="<c:url value='/mail.action!reply?user_id=${inbox.send_id }'/>" >
	                                                        <i class="fa fa-reply mail-icon"></i>
	                                                        	回复
	                                                    </a>
	                                                </li>
	                                                <c:if test="${inbox.isread  eq 0}">
	                                                	<li>
                                                    		<a href="<c:url value='/mail.action!setread?inbox_id=${inbox.pk_id }'/>" class="font-red" title="">
                                                        		<i class="fa fa-eye mail-icon"></i>
                                                        			设为已读
                                                    		</a>
                                                		</li>
	                                                </c:if>
	                                                ...
	                                                <li>
                                                    <a href="<c:url value='/mail.action!delete?inbox_id=${inbox.pk_id }'/>" class="font-red" title="">
                                    ...
                                    <div class="mail-right"><p><fmt:formatDate value="${inbox.sendtime }" pattern="yyyy-MM-dd HH:mm:ss"/></p></div>
                                    <div class="clearfix"> </div>
                                    <div id="collapseOne${inbox.pk_id }" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
                                        <div class="mail-body">
                                            <p>${inbox.message }</p>
	                        ...
	                        <div class="text-center">
	                            <nav>
	                                <ul class="pagination"> 
	                                    <li <c:if test="${requestScope.pagebean.isfirst}">class="disabled"</c:if> ><a href="<c:url value='/mail.action!tomail?pageindex=${requestScope.pagebean.preindex }&isread=${isread }&sended=${sended }'/>" aria-label="Previous"><i class="fa fa-angle-left"></i></a></li>
	                                    <c:forEach items="${requestScope.pagebean.pageList }" var="page"> 
	                                    	<li <c:if test="${requestScope.pagebean.pageindex eq page}">class="active"</c:if>><a href="<c:url value='/mail.action!tomail?pageindex=${page }&isread=${isread }&sended=${sended }'/>">${page }</a></li>
	                                    </c:forEach>
	                                    <li <c:if test="${requestScope.pagebean.islast}">class="disabled"</c:if>><a href="<c:url value='/mail.action!tomail?pageindex=${requestScope.pagebean.nextindex }&isread=${isread }&sended=${sended }'/>" aria-label="Next"><i class="fa fa-angle-right"></i></a></li>
	                                </ul>
	                            </nav>
	                        </div>
	                        </c:otherwise>
